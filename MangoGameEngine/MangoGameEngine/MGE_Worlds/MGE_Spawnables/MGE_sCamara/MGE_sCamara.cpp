...
Vector2f sCamera::GetVertexPojection(const Vector3f& world_vertex_location) const
{
    Vector3f local_vertex_location = world_vertex_location - this->world_location;
        
    Vector3f vertex_pojection_3D = Plane3fLine3fIntersection(this->GetLensNormal(), this->GetLocalLensLocation(), local_vertex_location);
    ...
    Vector2f rotated_vertex_pojection_2D = Vector2f(vertex_pojection_2D.GetLength(),
                                                    vertex_pojection_2D.GetPolarPhi() + this->world_rotation.x, true);
...
std::vector<Plane3f> sCamera::GetVisableTriangles(const std::vector<sObject>& scene)
{
    ...
    {
        for (auto triangle: object.mesh)
        {
            if (this->IsTriangleVisable(triangle))
            {
                Plane3f visable_triangle;
                
                for (int i = 0; i < 3; i++)
                {
                    visable_triangle.point[i] = triangle.point[i] - this->world_location;
                }
...
std::vector<Triangle2f> sCamera::ClipTriangle(const Triangle2f& triangle)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            if (screen_boundary[i].SignedShortestDistance(triangle.vertex[j]) > 0.0f)
            {
        ...
        switch (visable_vertices)
        {
                ...
                for (int j = 0; j < 3; j++)
                {
                    if(is_vertex_visable[j])
                    {
                        result.emplace_back(Triangle2f(triangle.vertex[j],
                                                       TwoLine2fIntersection(screen_boundary[i],Line2f(triangle.vertex[j],
            ...
            case 2:
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    {
                        result.emplace_back(Triangle2f(triangle.vertex[j],
                                                       TwoLine2fIntersection(screen_boundary[i],Line2f(triangle.vertex[(j - 1) % 3],
                                                                                                       triangle.vertex[(j + 1) % 3])),
                                                       TwoLine2fIntersection(screen_boundary[i],Line2f(triangle.vertex[(j - 1) % 3],
                                                                                                       triangle.vertex[j]))));
